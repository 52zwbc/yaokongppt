...
export function App() {
  ...
  const rawFit = useCallback(
    (s?: RenderSlide) => {
      ...
      // -24: 32px vertical padding minus the same 8px slack as the width; the AI bar
      // is gone, so the 48px headroom it used to reserve is no longer subtracted.
      const availH = viewportH - 24
  ...
  // ── AI scratch state (retained, inert: AI features were removed) ───────
  ...
  // 启动软件即默认开启远程控制服务（静默，不弹窗）；点「远程控制」按钮才显示控制面板。
  const startedRef = useRef(false)
  useEffect(() => {
    if (startedRef.current) return
    startedRef.current = true
    void window.slidesApi.startRemoteControl().then((info) => {
      setRemoteControl(info)
    }).catch(() => {
      /* 启动失败不阻塞主程序 */
    })
  }, [])
  // 订阅验证结果：granted=写入本地记录并激活；denied=验证未通过（仍可使用基础功能）
  useEffect(() => {
    const off = window.slidesApi.onLicenseEvent((ev) => {
      if (ev === 'granted') setLicenseBanner({ type: 'granted', msg: '已激活，感谢支持 LitePPT！' })
      else setLicenseBanner({ type: 'denied', msg: '手机号验证未通过，仍可正常使用基础功能' })
    })
    return off
  }, [])
  const toggleRemoteControl = useCallback(() => {
    if (remoteControl) {
      // 已在运行：切换控制面板的显示（点击按钮才显示）
      setRemoteControlModal((o) => !o)
    } else {
      void window.slidesApi.startRemoteControl().then((info) => {
        setRemoteControl(info)
        setRemoteControlModal(true)
        // 防盗版：未验证则打开验证网页（本次会话仅提示一次）；基础功能仍可正常使用
        if (!verifyPromptedRef.current) {
          verifyPromptedRef.current = true
          void window.slidesApi.checkLicense().then((ok) => {
            if (!ok) {
              setLicenseBanner({
                type: 'need',
                msg: '首次使用建议验证：请在新打开的网页输入手机号激活（网页打不开也不影响文件管理功能）',
              })
              void window.slidesApi.openLicenseVerify()
            }
          })
        }
      })
  ...
  useEffect(() => {
    void window.slidesApi.getAutoLaunch().then(setAutoLaunch).catch(() => {})
    void window.slidesApi.getAppVersion().then(setAppVersion).catch(() => {})
  }, [])
  const toggleAutoLaunch = useCallback(() => {
    const next = !autoLaunch
    setAutoLaunch(next)
    void window.slidesApi.setAutoLaunch(next).then(setAutoLaunch).catch(() => {})
  }, [autoLaunch])

  // ── 随机点名：订阅播放事件，用 <audio> 出声（主进程已弹浮窗）──
  useEffect(() => {
    const off = window.slidesApi.onRollCallPlay((ev) => {
      if (ev.sound) {
        try {
  ...
  return (
    <div className="app">
      ...
      <Ribbon
        ...
        onRemoteControl={toggleRemoteControl}
        remoteControlActive={!!remoteControl}
        ...
        <div className="app-content">
          ...
          <footer className="status-bar">
            <div className="status-left">
              ...
              <button
                className={`status-autolaunch${autoLaunch ? ' on' : ''}`}
                data-tip={autoLaunch ? '开机自启：已开启（点击关闭）' : '开机自启：已关闭（点击开启）'}
            ...
            <div className="status-center">
              <span className="status-credit">请用微信小程序《52中文编程》查看使用帮助和联系作者</span>
              <span className="status-ver">v{appVersion}</span>
            </div>
          ...
          {remoteControlModal && remoteControl && (
            <div className="rc-modal-mask" onClick={() => setRemoteControlModal(false)}>
              <div className="rc-modal" onClick={(e) => e.stopPropagation()}>
                <div className="rc-modal-title">放映远程控制</div>
                <div className="rc-modal-sub">
                  点下方某个地址查看其二维码；用手机或同网段电脑浏览器扫码/输入地址打开远程文件页
                  （上传/打开 PPTX），再进入遥控页（手机即变身远程键盘）。
                </div>
                {remoteControl.entries.length > 0 ? (
                  <div className="rc-addr-list">
                    {remoteControl.entries.map((e, i) => (
                      <button
                        key={e.url}
                        className={'rc-addr' + (i === rcSelected ? ' active' : '')}
                        onClick={() => setRcSelected(i)}
                      >
                        {e.url}
                      </button>
                    ))}
                  ...
                  <div className="rc-addr-list">
                    <button className="rc-addr active">{remoteControl.localUrl}（未检测到局域网地址）</button>
                  </div>
                )}
                {remoteControl.entries[rcSelected] ? (
                  <>
                    <img className="rc-modal-qr" src={remoteControl.entries[rcSelected].qr} alt="远程控制二维码" />
                    <div className="rc-modal-url">{remoteControl.entries[rcSelected].url}</div>
                  </>
                ) : (
                  <div className="rc-modal-url">{remoteControl.localUrl}</div>
                )}
                <div className="rc-modal-tip">
                  若手机长时间打不开：在 Windows 防火墙“允许应用通过防火墙”中勾选本程序（专用 + 公用网络）。
                  按键由系统发给“最前方窗口”，请确保目标窗口已聚焦。
                  上传的文件保存在电脑“文档\\pptx演示器”文件夹（远程页可查看实际位置）。
                </div>
                {licenseBanner && (
                  <div className={'rc-modal-lic rc-modal-lic-' + licenseBanner.type}>{licenseBanner.msg}</div>
                )}
                <div className="rc-modal-actions">
                  <button
                    className="rc-modal-close"
                    onClick={() => setRemoteControlModal(false)}
                  >
                    知道了
                  </button>
                </div>
