/**
 * pptx演示器 主进程 — pptx parsing/render-tree building/edit application/saving all live
...
import { registerRemoteControlIpc, setRemoteOpenHandler, handleLicenseProtocol } from './remote-control'
import { registerRollCallIpc } from './roll-call'
...
/**
 * Recovery drafts for never-saved decks (wcId → visible path in <Documents>/pptx演示器):
...
/** Directory where unsaved drafts are saved: the configurable default save folder (falls back to <Documents>/pptx演示器) */
...
/**
 * Auto-save the draft to <Documents>/pptx演示器/<name>.pptx.
...
.page img { display: block; width: 100%; height: 100%; }
  ...
  // ── 放映远程控制（局域网手机/电脑浏览器控制）──
  registerRemoteControlIpc()
  registerRollCallIpc()

  registerAppLifecycleIpc()
  // 远程文件页“点击文件在软件内打开”：复用 open-file 的同一机制（openAndBuild + slides:opened）
  setRemoteOpenHandler(async (p) => {
    const win = BrowserWindow.getAllWindows().find((w) => !w.isDestroyed())
    if (!win) return false
    try {
      const r = await openAndBuild(win.webContents, p, 1280)
...
/** Linux 自启文件名：位于 ~/.config/autostart 的 .desktop（即“开机随系统启动”）。 */
const AUTO_LAUNCH_DESKTOP = '遥控PPT.desktop'

/** Linux 上 app.setLoginItemSettings 无效（Electron 仅支持 mac/win），改用手写 autostart .desktop。 */
function linuxAutoLaunchPath(): string | null {
  if (process.platform !== 'linux') return null
  try {
    return join(app.getPath('home'), '.config', 'autostart', AUTO_LAUNCH_DESKTOP)
  } catch {
    return null
  }
}
...
function readAutoLaunch(): boolean {
  if (process.platform === 'linux') {
    const p = linuxAutoLaunchPath()
    return !!p && existsSync(p)
  }
  return app.getLoginItemSettings().openAtLogin
}

function writeAutoLaunch(on: boolean): boolean {
  if (process.platform === 'linux') {
    const p = linuxAutoLaunchPath()
    if (p) {
      if (on) {
        try {
          ...
          console.warn('[slides] 创建开机自启 .desktop 失败：', (e as Error)?.message)
        }
      } else if (existsSync(p)) {
        try {
          unlinkSync(p)
        } catch (e) {
          console.warn('[slides] 删除开机自启 .desktop 失败：', (e as Error)?.message)
        }
      }
    }
...
/**
 * 开机自启：首次运行默认开启（用 userData 下的标记文件保证“只默认一次”，
 * 之后完全由用户在状态栏开关控制，不会被再次强行打开）。
 */
function initAutoLaunchDefault(): void {
  try {
    const flag = join(app.getPath('userData'), 'auto-launch-initialized.json')
    if (existsSync(flag)) return
    mkdirSync(dirname(flag), { recursive: true })
    writeFileSync(flag, JSON.stringify({ at: new Date().toISOString() }), 'utf8')
    writeAutoLaunch(true)
  } catch (e) {
    console.warn('[slides] 初始化开机自启失败：', (e as Error)?.message)
  }
}

function registerAppLifecycleIpc(): void {
  ipcMain.handle('slides:auto-launch-get', () => readAutoLaunch())
  ipcMain.handle('slides:auto-launch-set', (_e, on: boolean) => writeAutoLaunch(!!on))
  ipcMain.handle('slides:app-version', () => app.getVersion())
}
...
// ── 系统托盘：关闭窗口改为最小化到托盘，继续提供无线遥控服务；退出需右键托盘图标 ──
let tray: Tray | null = null
let isQuitting = false
let mainWin: BrowserWindow | null = null

/** 解析应用图标（托盘/窗口用），按 打包 resources > 开发目录 优先级；找不到则返回空 nativeImage，避免 Tray 初始化崩溃。 */
...
/** 托盘图标：优先使用 PNG（Linux 托盘对 ICO 支持差，会导致显示空白）。 */
...
/** 创建托盘图标与右键菜单（显示 / 退出）。已存在则跳过。 */
function ensureTray(): void {
  if (tray) return
  const showWin = showMainWin
  const menu = Menu.buildFromTemplate([
    { label: '显示', click: showWin },
    { label: '退出', click: () => quitApp() },
  ])
  tray = new Tray(appIconImage())
  tray.setToolTip('遥控PPT')
  tray.setContextMenu(menu)
  // 单击托盘图标等同“显示”
  tray.on('click', showWin)
}

/** 真正退出：若有未保存改动先走存盘提示，确认后再退出。 */
function quitApp(): void {
  if (isQuitting) return
  const w = mainWin
  if (w && !w.isDestroyed() && slidesIsDirty(w.webContents.id)) {
    void requestSlidesClose(w.webContents, w).then((proceed) => {
      if (proceed) {
        isQuitting = true
        app.quit()
      }
...
export function createSlidesWindow(openPath?: string | null): BrowserWindow {
  const win = new BrowserWindow({
    ...
    // 窗口/任务栏图标：用应用自身图标，否则 Linux 上显示默认齿轮
    ...(process.platform !== 'darwin' && appIconPath()
      ? { icon: appIconPath() as string }
      : {}),
    webPreferences: {
  ...
  // 关闭按钮：不退出程序，而是最小化到系统托盘，继续提供无线遥控服务；退出需右键托盘图标。
  // 真正退出时（托盘“退出”调用 app.quit()）isQuitting=true，此处放行。
  win.on('close', (event) => {
    if (isQuitting) return
    event.preventDefault()
    ensureTray()
    if (slidesIsDirty(win.webContents.id)) {
      void requestSlidesClose(win.webContents, win).then((proceed) => {
        // 存盘/放弃后隐藏到托盘；取消则保持窗口打开
        if (proceed && !win.isDestroyed()) win.hide()
      })
      return
    }
    win.hide()
...
async function applyMainProcessProxy(): Promise<void> {
  ...
  try {
    await app.whenReady()
    // PAC/rule proxies answer per-host: probe a public host to discover the system proxy
...
export function startSlidesStandalone(): void {
  ...
  // 信息呼叫浮窗的 <audio> 需无用户手势自动播放，放宽媒体自动播放策略（对本应用无副作用）
  ...
  // 单实例模式：已运行实例收到“再次双击图标 / 打开方式”时，复用当前实例（唤起窗口并打开 pptx），不再新开进程。
  // 测试模式（GENOFFICE_USER_DATA）已切换 userData，锁作用于该 userData，故放在 setPath 之后。
  ...
  // 单实例模式：双击图标 / 右键“打开方式”均复用当前实例（见下方 second-instance 处理）；远程控制服务端口冲突会自动换端口重试。
  // 防盗版：注册 liteppt:// 协议，接收验证网页回跳（liteppt://verified?..  / liteppt://denied）
  app.setAsDefaultProtocolClient('liteppt')
  app.on('open-url', (_e, url) => {
    if (url.startsWith('liteppt://')) handleLicenseProtocol(url)
  })
  ...
  // 单实例：双击图标 / 右键“打开方式”再次启动时，复用当前实例（唤起窗口并用其打开 pptx）
  app.on('second-instance', (_e, argv) => {
    // 网页 liteppt:// 协议回跳（验证页在本实例上唤起）
    for (const a of argv) {
      if (a.startsWith('liteppt://')) handleLicenseProtocol(a)
    }
    // 用当前实例打开 pptx，而非新开进程
    let pptx: string | undefined
    for (const a of argv) {
      if (a.toLowerCase().endsWith('.pptx') && existsSync(a)) { pptx = a; break }
    }
    showMainWin()
    if (pptx && mainWin && !mainWin.isDestroyed()) {
      openAndBuild(mainWin.webContents, pptx, 1280)
        .then((r) => mainWin!.webContents.send('slides:opened', r))
        .catch(() => {})
    }
  })
  ...
  // 防盗版：首实例直接被协议唤起时（如点击网页里的 liteppt:// 链接），argv 中带回调 URL
  ...
  app.whenReady().then(async () => {
    ...
    initAutoLaunchDefault() // 首次运行默认开启开机自启（之后由状态栏开关决定）
    registerSlidesIpc()
    registerProjectIpc()
    Menu.setApplicationMenu(buildSlidesMenu())
  ...
  app.on('window-all-closed', () => {
    // 有托盘时：窗口关闭只是隐藏到托盘，应用继续运行（提供无线遥控）；仅真正退出时才退出
