...
const api: SlidesApi = {
  ...
  // ── 放映远程控制（局域网）──
  startRemoteControl: () => ipcRenderer.invoke('slides:remote-control-start'),
  stopRemoteControl: () => ipcRenderer.invoke('slides:remote-control-stop'),
  // ── 防盗版（手机号验证 + 机器绑定本地记录）──
  checkLicense: () => ipcRenderer.invoke('slides:license-check'),
  openLicenseVerify: () => ipcRenderer.invoke('slides:license-open-verify'),
  onLicenseEvent: (handler: (ev: 'granted' | 'denied') => void) => {
    const listener = (_e: IpcRendererEvent, ev: 'granted' | 'denied') => handler(ev)
    ipcRenderer.on('slides:license-event', listener)
    return () => ipcRenderer.removeListener('slides:license-event', listener)
  },
  // ── 开机自启 / 版本号（状态栏）──
  getAutoLaunch: () => ipcRenderer.invoke('slides:auto-launch-get'),
  setAutoLaunch: (on: boolean) => ipcRenderer.invoke('slides:auto-launch-set', on),
  getAppVersion: () => ipcRenderer.invoke('slides:app-version'),
  // ── 随机点名 ──
  rollCallPick: () => ipcRenderer.invoke('slides:roll-call-pick'),
  // ── 放映导航“显示桌面”（Win+D，便于演示中切换程序）──
  showDesktop: () => ipcRenderer.invoke('slides:show-desktop'),
  onRollCallPlay: (handler: (ev: { name: string; sound: string | null }) => void) => {
    const listener = (_e: IpcRendererEvent, ev: { name: string; sound: string | null }) => handler(ev)
    ipcRenderer.on('slides:roll-call-play', listener)
    return () => ipcRenderer.removeListener('slides:roll-call-play', listener)
  },
}
