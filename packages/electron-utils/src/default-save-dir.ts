...
/// save and where drafts go. Historically hardcoded to
/// <Documents>/pptx演示器; now user-configurable via the `defaultSaveDir` key
/// in userData/app-settings.json.
...
export function configuredDefaultSaveDir(app: PathProvider): string {
  ...
  const fallback = join(app.getPath('documents'), 'pptx演示器')
